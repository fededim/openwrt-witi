...
)
{
    ...
    printk("Adapter_Interrupts_Top_Half_Handler: starts");
        ...
        Adapter_InterruptHandler_t HandlerFunction)
{
    ...
    if (Adapter_Interrupts_IRQ != -1)
    {
        ...
        printk("Adapter_Interrupt_SetHandler: Unable to set handlerFnc=%p for interrupt %d (irq not hooked %d)\n", HandlerFunction, nIRQ,Adapter_Interrupts_IRQ);
}
...
void mtk_BH_handler_resultGet(unsigned long data) {
	printk("mtk_BH_handler_resultGet invoked..");
}
 ...
 */
bool Adapter_Interrupts_Init(const int nIRQ)
{
...
#ifdef RT_EIP93_DRIVER
	//printk("Adapter_Interrupts_Init: before set_vi_handler");
        //set_vi_handler(nIRQ, crypto_irq_dispatch);
	//printk("Adapter_Interrupts_Init: after set_vi_handler");
#endif
        ...
        if (res!=0)
        {
            printk("Adapter_Interrupts_Init: request_irq for irq %d fails, returned %d\n",nIRQ,res);

	    return false;
        }
        else //success 
        {
            printk("Adapter_Interrupts_Init: Successfully hooked IRQ %d\n",nIRQ);
