 ...
 *     false Failed
 */
bool Adapter_EIP93_SetMode_ARM(const bool fEnableDynamicSA)
{
...
/**** allocate memory for the ring ****/
...

    {
        ...
        if (dmares != DMABUF_STATUS_OK)
        {
            printk(
                "Adapter_EIP93_Init: "
                "DMABuf_Alloc (Command Ring) error %d\n",
                dmares);
...
//    Settings.nRingPollDivisor = ADAPTER_EIP93_RINGPOLLDIVISOR;
    ...
    printk("Adapter: Successfully initialized EIP93v2 in ARM mode (EIPDevice %x)\n",EIP93Device);
 ...
 * Adapter_EIP93_Init
 */
bool Adapter_EIP93_Init(void)
{
    EIP93_Status_t res93;

    Adapter_EIP93_Mode = ADAPTER_EIP93_MODE_DISABLED;

    if (!HWPAL_Device_Find("eip93", &EIP93Device))
    {
        printk("Adapter_EIP93_Init: Failed to find EIP93 device\n");
        return false;
    }

    res93 = EIP93_Initialize(&Adapter_EIP93_IOArea,EIP93Device);

    if (res93 != EIP93_STATUS_OK)
    {
        printk("Adapter_EIP93_Init: EIP93_Initialize error %d\n",res93);
        return false;
    }

#ifdef ADAPTER_EIP93PE_INTERRUPTS_ENABLE
    // Configure level of Interrupts
    res93 = EIP93_INT_Configure(&Adapter_EIP93_IOArea,false,false);

    if (res93 != EIP93_STATUS_OK)
    {
        printk("Adapter_EIP93_Init: EIP93_INT_Configure error %d\n",res93);

        return false;
    }

    printk("Adapter_EIP93_Init: EIP93_INT_Configure successfully configured level\n");

#endif // ADAPTER_EIP93PE_INTERRUPTS_ENABLE

    Adapter_EIP93_Mode = ADAPTER_EIP93_MODE_IDLE;

    printk("Adapter_EIP93_Init: successfully initialized IDLE MODE\n");
...
Adapter_EIP93_UnInit(void)
{
    ...
    printk("Adapter_EIP93_UnInit: shutting down..");
